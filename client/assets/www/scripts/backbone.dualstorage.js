(function() {
  ...
  window.Store = (function() {
	...
	On crée un Index
	*/
    Store.prototype.createIndex = function(model) {
      console.log('creatingIndex ', model, 'in', this.name);
	  if (!_.isObject(model)) return model;
      if (!model.id) {
        //generate False Id
        model.id = this.generateId();
      ...
      //work only with id !
      localStorage.setItem(this.name, JSON.stringify(model));
      //this.records.push(model.id.toString());
      //this.save();
    ...
    Store.prototype.findAll = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        id = _ref[_i];
        console.log('findAlling  '+this.name + this.sep + id);
    ...
    response = (function() {
      switch (method) {
        ...
        case 'hasDirtyOrDestroyed':
          //console.log('local Sync hasDirtyOrDestroyed');
          return store.hasDirtyOrDestroyed();
        case 'clear':
          //console.log('local Sync clear');
          return store.clear();
        case 'create':
          //console.log('local Sync create');
  ...
  dualsync = function(method, model, options) {
    ...
    // + '|' + (obj.id || GUID())
    // si creation est offline : on va génerer un FAUX ID
